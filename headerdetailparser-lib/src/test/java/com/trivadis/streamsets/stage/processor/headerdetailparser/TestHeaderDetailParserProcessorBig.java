...
public class TestHeaderDetailParserProcessorBig {
	private final static int NOF_HEADER_LINES = 15;
	private final static String TEST_FILE_WITH_HEADER_AND_DETAILS_HEADER = "bigfile_with_header_and_details_col_header.txt";
	private final static String TEST_FILE_WITH_HEADER_AND_DETAILS_HEADER_BIGANDWIDE = "small-test-wide.csv";
...
//		assertEquals("11:02:12.000", StringUtils.substring(op.get(0).get("/detail").getValueAsString(), 0, 12));
	...
	@Test
	@SuppressWarnings("unchecked")
	public void test_headerSplitOnNofLinesWithRegex_BigAndWide() throws StageException, IOException {
		...
		List<Record> op = null;
		try {
			List<Record> input = prepareInput(TEST_FILE_WITH_HEADER_AND_DETAILS_HEADER_BIGANDWIDE);
		    
			long startTime = System.currentTimeMillis();
			
			StageRunner.Output output = runner.runProcess(input);
			
		    System.out.println("test_headerSplitOnNofLinesWithRegex_BigAndWide - Total execution time: " + (System.currentTimeMillis()-startTime) + "ms"); 
