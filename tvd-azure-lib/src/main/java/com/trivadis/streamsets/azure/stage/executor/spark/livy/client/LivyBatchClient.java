 ...
 */
package com.trivadis.streamsets.azure.stage.executor.spark.livy.client;
...
public class LivyBatchClient {
	...
	public Session createJob(BatchJobParameters req) throws IOException, LivyException {
		...
		con.setRequestProperty("X-Requested-By", user);
        ...
        try {
        	...
        	if(resp != 200 && resp != 201) {
        		...
        		String errorMsg = StringUtil.getStreamAsString(es, charset);
        		throw new LivyException("Invalid HTTP response code (" + resp + ") is returned. " + errorMsg);
        	}
